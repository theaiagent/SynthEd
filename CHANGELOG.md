...
### Fixed
- **`engagement_gpa_correlation` validation test silently never ran** (audit, Finding 1): `_prepare_validation_data` emitted only `final_engagement` in each outcome dict, but `SyntheticDataValidator` reads `o.get("mean_engagement")` for the engagement–GPA Pearson test. The field was always `None`, so the `len(eng_xs) > 10` guard was never satisfied and the engagement–GPA test never appeared in the validation report (proven by the new regression test, which failed before the fix). Fix adds a `mean_engagement` field — the semester mean of `weekly_engagement_history` — to the outcome dict. Mean rather than final-week engagement is used because final-week engagement is a single noisy snapshot truncated at each student's dropout week (so history length varies between students), whereas the semester mean is a stable, low-variance summary and the empirically stronger GPA predictor (the engine truncates history at dropout; it does not zero-pad, so there is no "~0 final-week" artifact). New regression test `test_pipeline_validation_includes_engagement_gpa_correlation`.
- **Sobol failed-sample handling no longer contaminates sensitivity indices** (audit, Finding 5; hardened per CodeRabbit): the parallel branch of `SobolAnalyzer._run_simulations` previously zero-filled any sample whose worker raised (`dropout_rate=0.0, mean_engagement=0.0, …`), blending fabricated zeros into the Saltelli matrix and silently skewing the S1/ST indices. Saltelli's rigid `n*(D+2)` row order forbids dropping a row, so the fix instead re-runs every sample that has no result. After the parallel pass, any slot still `None` — the worker raised, the process died (`BrokenProcessPool`), or the pass hit its time budget — is retried in isolation on a *fresh* single-worker executor (so a pool broken by one worker death cannot doom the whole run), up to `_MAX_SAMPLE_RETRY_ATTEMPTS` (2) times, then `RuntimeError` if still failing. Behaviour change: an unrecoverable sample aborts the run instead of returning contaminated indices. New `_result_with_retry` + `_run_sample_isolated` helpers, covered by four unit tests and four real-subprocess integration tests (worker-death recovery, fail-fast, output ordering, and timeout→isolated-retry recovery).
- **`TheoryModule` protocol docstring corrected** (audit, Finding 3): it stated that engagement-composition theories define `_PHASE_ORDER`; they define `_ENGAGEMENT_ORDER` (enforced in `engine.py`). `_PHASE_ORDER` governs phase-method *discovery* ordering. The docstring now documents both attributes accurately.
- **README & GUIDE "zero engine changes" claim narrowed** (audit, Finding 2): both implied any new theory needs no engine edit. Only phase-method theories are auto-discovered; the six engagement-composition theories (Bean, Kember, Moore, Rovai, Gonzalez, PositiveEvents) are still manually instantiated in `engine.py` (tracked by an in-code TODO). Wording in both docs now distinguishes the two cases.
- **PyPI classifier aligned with development stage** (audit, Finding 4): `Development Status :: 5 - Production/Stable` → `4 - Beta`, consistent with the README's "under active development — APIs and output formats may change between versions."
- **CI dependency caps** (`build`): pin `numpy<2.5` and `scipy<1.18` in `pyproject.toml`. scipy 1.18.0's array-API `ks_1samp` passes `where` positionally to `ndtr`, which numpy 2.5.0 rejects (`TypeError: ndtr() takes from 1 to 2 positional arguments but 3 were given`), breaking `stats.kstest`/`pearsonr` across the validation suite — CI's unbounded `scipy>=1.10.0`/`numpy>=1.24.0` had started resolving to that broken combo. scipy 1.17.x + numpy 2.4.x verified green against the full suite; lift the caps once fixed upstream.
...
- **Auto-generated test inventory table** (issue [#87](https://github.com/theaiagent/SynthEd/issues/87)): `doc_facts.py` now auto-generates the `docs/THEORY.md` Test Suite inventory table from live test discovery. Sentinel comments (`<!-- BEGIN:test_inventory -->` / `<!-- END:test_inventory -->`) demarcate the table region. The Coverage column is extracted from each test file's module-level docstring via AST. Running `doc_facts --fix` regenerates the table; `--check` fails CI if it diverges. Three test files that lacked module docstrings (`test_institutional_config.py`, `test_nsga2_calibrator.py`, `test_pareto_utils.py`) gained them. Eliminates the recurring docs-drift source identified in the Phase 1.5 audit (13 drift points, 73 tests worth of stale table entries).
...
- **SSQ integration test thresholds empirically calibrated** (issue [#86](https://github.com/theaiagent/SynthEd/issues/86)): 50-seed study (seeds 41–90, n=200) measured paired per-seed dropout effects: high-SSQ mean=0.0482 (p10=0.0140), low-SSQ mean=0.0687 (p10=0.0295). Both p10 ≥ 0.012 → `eps` lowered from 0.010 to 0.008 with >90% power; seed count unchanged at 10. `docs/CALIBRATION_METHODOLOGY.md` §7.2 expanded with measured distributions. Measurement script: `scripts/measure_ssq_effect.py`.
