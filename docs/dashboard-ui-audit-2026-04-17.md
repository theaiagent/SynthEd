...
### Accessibility
- **P1-3** (resolved in this PR): Status text "N=200, seed=42, 1 sem" rendered at ~1.88:1 on the dark navbar at audit time (WCAG AA FAIL). The root cause was not the `TEXT_SECONDARY` value itself — at `#94A3B8` it already carried 7.4:1 on `--bg` — but Bootstrap's default `.text-secondary` color (roughly `rgb(64,64,64)`) was winning the cascade on the specific `.text-end.text-secondary` selector used by the status text, because our custom CSS did not set a `color` rule on that selector. The fix adds `color: var(--text-secondary) !important` on `.text-end.text-secondary` so the status text picks up our variable, and raises `TEXT_SECONDARY` to `#B0BCC8` (9.8:1 on `--bg`, 8.7:1 on `--surface`, 10.1:1 on navbar bg) for consistent above-AA margin across all custom `var(--text-secondary)` usages. Light-mode secondary text at `#475569` on `#F8FAFC` is ~7.2:1 (already passing — no change).
